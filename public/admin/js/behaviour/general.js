var App = function () {
    ...
    var textEditor = function () {
        /*//Ckeditor
         ...
         filebrowserUploadUrl: '/admin/gallery/upload',
         'extraPlugins' : 'iframe',
         'language': 'tr'
         });
        ...
        tinymce.init({
            selector: "textarea",
            height: 400,
            toolbar: "refbuton | pink | cat | pasteword |insertfile | styleselect | bold italic | alignleft aligncenter alignright alignjustify | bullist  | link image jbimages",
            plugins: [
                "advlist autolink link image lists charmap print preview hr anchor pagebreak spellchecker",
                "searchreplace wordcount visualblocks visualchars code fullscreen insertdatetime media nonbreaking",
                "save table  wordcount contextmenu directionality emoticons template paste textcolor jbimages"
            ],
            setup: function (editor) {
                editor.addButton('refbuton', {
                    text: 'Ref',
                    icon: false,
                    onclick: function () {
                        editor.focus();
                        editor.selection.setContent('<span class="referans" style="font-size: 10px">' + editor.selection.getContent() + '</span>');
                    }
                });

                editor.addButton('pink', {
                    text: 'Pink',
                    icon: false,
                    onclick: function () {
                        editor.focus();
                        editor.selection.setContent('<span class="pink" style="color:#ea158c">' + editor.selection.getContent() + '</span>');
                    }
                });

                editor.addButton('cat', {
                    text: 'Cat',
                    icon: false,
                    onclick: function () {
                        editor.insertContent('<p class="devil-icon" style="background: url(\'http://ekranella.92lab.me/a/img/ahmetkafasi.png\') no-repeat; float: left; width: 30px; height: 30px; padding-left: 0px; margin-right: 5px; font-size:0px;">.</p>');
                    }
                });
    ...
    return {
        ...
        init: function (options) {
            ...
            /*Tags*/
            if (config.tags) {
                var tagsSelect2 = $(".tags").select2({
                    tags: 0, width: '100%', tokenSeparators: [","],
...
$(function () {
    $(window).bind("beforeunload", function (e) {
        $("body").animate({opacity: 0, 'margin-left': -50}, 300, function () {
            return true;
        });
    });
...
$(document).ready(function () {
    ...
    $('a[data-toggle="tab"]').on('click', function (e) {
        var savedTemp = JSON.parse(localStorage.getItem('lastTabs'));
        ...
        saved[$(this).attr('data-tabname')] = $(this).attr('href');

        localStorage.setItem('lastTabs', JSON.stringify(saved));
    });

    var lastTabs = JSON.parse(localStorage.getItem('lastTabs'));
    if (lastTabs != undefined) {
        $.each(lastTabs, function (key, value) {
            $('a[data-tabname="' + key + '"][href="' + value + '"]').trigger('click');
        });
    }

    $(document).on('change', '.title', function () {
        var value = $(this).val();
        var permalink = value.toPermalink();
...
String.prototype.toPermalink = function () {
    var chars = {"ç": "c", "ö": "o", "ğ": "g", "ü": "u", "ş": "s", "ı": "i"};
    return this.toLocaleLowerCase().replace(/[çöğüşı]/g, function (char) {
        return chars[char];
