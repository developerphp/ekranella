...
<div class="page-head">
<h2>
<a href="@if($type == 1) {{action('NewsController@getIndex')}} @elseif($type == 2) {{action('NewsController@getSpecialNews')}} @else {{action('NewsController@getPortrait')}} @endif" class="btn btn-primary btn-flat btn-sm"><i class="fa fa-backward"></i></a>
{{$title}} - Galeri
</h2>
<ol class="breadcrumb">
  ...
  @else
  <li><a href="{{action('NewsController@getPortrait')}}">Portre Haberler</a></li>
  @endif
    ...
    <div class="block-flat">
        ...
        <div class="row no-margin">
          <div class="box-content" id="inbox">
          	<ul class="orderlist" style="list-style: none" id="arkaplansiralama" >
          	        ...
          	        <span>
                           ...
                           <div style="float: right; width: 40%">{{str_limit($item['text'], 300)}}</div>
                            ...
                            <a href="{{action('GalleryController@getDelete',['id'=> $item['id'], 'news' => $id])}}" onclick="return confirm('Görsel silinecek!')"  class="btn btn-danger btn-flat btn-xs"><i class="fa fa-times"></i> </a>
                            </div>
            ...
            <div class="col-md-5">
                <div class="cl-mcont">
                      <form action="{{action('NewsController@postUpload')}}"
                      class="dropzone"
                ...
                function saveItem(img){
                    $.post('{{action('GalleryController@postAddItem')}}',{'img':img, 'item':'{{$id}}', 'enum': {{\Config::get('enums.news')}} },function(response){
