            ...
            <div class="block-flat">
                ...
                <form method="post" enctype="multipart/form-data">
                    ...
                    <div class="row no-margin">
                        <div class="col-sm-8">
                            <div class="form-group">
                                <label class="control-label">Bölüm Başlığı</label>
                    ...
                    <div class="row no-margin">
                        <div class="col-sm-8">
                            <div class="form-group">
                                ...
                                ?>
                                @if(isset($item->airing_date) && $item->airing_date != "")
                                    <?php $date = $item->airing_date;
                                    $use = explode(' ', $date); ?>
                                @else
                                    <?php $use = ['', '', ''] ?>
                                @endif
                                <label class="control-label">Bölümün Yayınlanma Tarihi</label>

                                <div class="row dateDiv">
                                    <div class="col-sm-3">
                                        <label class="control-label">Gün</label>
                                        <input type="number" min="1" max="31" class="form-control" name="day_number"
                                               value="{{trim($use[0])}}"/>
                                    </div>
                                    <div class="col-sm-3">
                                        ...
                                        <select name="month" class="select2">
                                            @foreach($months as $month)
                                                <option value="{{$month}}" @if($month == trim($use[1]))
                                                        selected @endif >{{$month}}</option>
                                            @endforeach
                                        </select>
                                    </div>
                                    <div class="col-sm-4">
                                        <label class="control-label">Haftanın Günü</label>
                                        <select name="day" class="select2">
                                            @foreach($days as $day)
                                                <option value="{{$day}}" @if($day == trim($use[2]))
                    ...
                    <div class="row no-margin">
                        <div class="col-sm-8">
                            <div class="form-group">
                                ...
                                <input type="text" name="number" min="1" class="form-control"
                                       value="@if(Input::get('number')){{{Input::get('number')}}}@endif{{{$item->number}}}"/>
                            </div>
                        </div>
                    </div>
                    <div class="row no-margin">
                            ...
                            <script>
                                $(document).ready(function () {
                                    $('.tumblrBtn').click(function () {
                                        ...
                                        $.get("{{action('SerialController@getEpisodeInfos')}}", {'url': $('.tumblr').val().trim()}, function (data) {
                                            $('.title').val(data.title);
                                            $('.tags').val(data.tags).trigger("change");
                                            $('.title').change();
                                            $('.datetime').val(data.date).change();
                        ...
                        @if(\admin\RoleController::isAdmin())
                            <div class="row no-margin author_alias" @if(!($item->is_author || $item->is_author == ""))
                                ...
                                <div class="col-sm-8">
                                    <div class="form-group">
                                        <label class="control-label">Bu içeriği bir başka yazara ata (Opsiyonel)</label>
